...
export class AuthService {
  ...
  async register(input: {
    ...
    const session = await this.createSession(result.id, input.userAgent, input.ipAddress);
  ...
  async forgotPassword(input: { email: string }) {
    ...
    if (!user?.passwordHash) {
      return {
        success: true,
        message: "Se existir uma conta com esse e-mail, enviaremos um link de redefinicao."
      };
    }
  ...
  async resetPassword(input: { token: string; password: string }) {
    ...
    if (
      ...
      throw new BadRequestException("O link de redefinicao esta invalido ou expirou.");
    }
  ...
  private validateIdentity(input: { email: string; fullName: string; password: string }) {
